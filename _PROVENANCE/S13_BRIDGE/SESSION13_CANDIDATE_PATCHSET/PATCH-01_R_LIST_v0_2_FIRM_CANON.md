# PATCH-01 — R-list v0.2 / CONFIG B FIRM CANON — Candidate Persisted Artifact
**Status: CANDIDATE (rung: proposed). NOT canon until operator force-close. Additive file; reversible by deletion.**
**Work item:** REPAIR.P1N.SESSION-TO-MAESTRO-BRIDGE.R1 · 2026-07-19
**What this is:** the session-13 operator-ratified FIRM canon (R-list v0.2, 96 destination requirements, 92 FIRM), reconstructed as a persisted artifact from the U20 verbatim record. V1 established this content existed in transcripts only; this file is the repair candidate.

## Provenance & anchors
- Payload = the FULL text of window-1 operator segment U20 (segment index 20), verbatim, zero alterations. Source: `artifacts/D-Maestro/sources/claud3.txt` (DEC-02 primary, md5 `faecdb760a6248fd08d9cce4e6e738ca`).
- Segment span: normalized chars 232261–256484 (CRLF→LF normalization per MF-1; segments.jsonl offsets). R-LIST header at RAW byte 244776.
- Payload integrity: md5 of the verbatim payload below = `52381c1e13a25775f5491a7090a7f06d` (24223 chars).
- Authority chain (all raw-byte anchors): U20 = operator relay of the operational window's R-list v0.2 (operator-authored act of selection; first line is quote-back). Ratified FIRM by W2 U53 operator F (Config B selection) and locked by W2 U56: "R-list v0.2 (CONFIG B FIRM CANON) received without challenge" @ `claude-session13-window2.txt` byte 883360; "Authority placed per R-RT-QAF-01" @ byte 883423; "Phantom-resolution audit method SUPERSEDED by R-RT-PD-01" region @ byte 884652. (W2 md5 `94989f7f37f67e6d1a818a1b974c1efc`.)
- Known export chrome inside the payload (flagged, NOT removed — MF-4 discipline): duplicated preview first line; `Show less`; trailing `9:36 PM` timestamp. Column whitespace preserved as-is.
- Contradictions & open items carried, not closed: OPEN-Q5–Q8 remain OPEN inside the payload; W-axis "NO ARTIFACTS" lines are window-scoped per U56 (C3 disposition); CR-2 six-verb tension preserved in its own register.

## Verbatim payload (U20, complete)
```text

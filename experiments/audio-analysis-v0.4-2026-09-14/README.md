# Maestro Audio Analysis v0.4 — Experimental Checkpoint

**Date:** 2026-09-14  
**Doctrine:** Your Vision. Our Mission.  
**Scope:** experimental evidence/integration checkpoint only  
**Parent main:** `e7a6e3671987bd53c2d64ce0155bc8afcee60777`

This checkpoint persists the current session's approved audio-analysis direction without promoting experimental model outputs into Technical UST canon or changing the accepted workforce/runtime authority.

## Approved authority boundary

`deterministic measurement -> labeled model inference -> operator listening judgment`

No experimental analyzer may self-promote a keeper, release/master approval, renderer policy, Technical UST mutation, or universal quality score.

## v0.4 trust-boundary repairs

1. Experimental adapter outputs use strict structural contracts; malformed or undeclared fields fail closed.
2. External bridges receive read-only source snapshots instead of preserved source paths.
3. Preserved originals and bridge snapshots are hashed before and after bridge execution; mutation invalidates the run.
4. Fast analysis no longer reports an unmeasured true-peak check as PASS. `AUDIO.TRUE_PEAK.001` is `not_assessed` when true peak was not measured.
5. Deterministic analysis/comparison performs source-hash postchecks.
6. FastAPI audio request models were moved to module scope and endpoint regression coverage was added.
7. Preserved parallel v0.4 work remains archived rather than overwritten; active pytest discovery is scoped to current `tests/`.

## Experimental components delivered in the session package

- Beat This beat/downbeat/tempo adapter + reference bridge
- SongFormer structure adapter + reference bridge
- ChordMini harmony adapter + reference bridge
- Basic Pitch baseline adapter
- tsumugi advanced instrument-agnostic AMT adapter + independent MIDI validation
- 48-D statistical acoustic descriptor retained as a non-semantic calibration baseline
- LAION-CLAP embedding adapter
- corpus-specific embedding calibration with minimum-sample gate, balanced-accuracy candidate threshold, and jackknife stability
- OpenAI structured evidence-reasoning adapter
- generic external direct-audio language-model bridge boundary

Heavy model environments/checkpoints are intentionally isolated from `runtime/maestro-workforce/`. Missing adapters report `not_configured`; they do not block deterministic analysis or unrelated adapters.

## Validation

Current package test result: **64 passed, 2 third-party Python 3.13 deprecation warnings**.

Validated gates include malformed-adapter rejection, source/snapshot immutability, fast true-peak `not_assessed`, public adapter aliases, statistical embedding shape/provenance, embedding calibration non-promotion, API compatibility routes, deterministic-analysis prerequisite, and a controlled successful experimental bridge job.

The current Bitter Thank You strict suite executed the built-in statistical descriptor successfully while reporting Beat This, SongFormer, ChordMini, Basic Pitch, advanced AMT, CLAP, and audio-language bridges as `not_configured` on this host. The known silent Contrast artifact failed closed during reconciled calibration. No heavyweight model execution is claimed.

## Session artifact

Reconciled package: `Maestro_Develop_v0_4_RECONCILED.zip`  
SHA-256: `b24b1aeb76e2cc711ecd017ee9624be7df798ec7ddf5aa664c38f94872e62a27`  
Package manifest contains 231 files and preserves the user-supplied parallel v0.4 branch under `source_baseline/user_v0_4_parallel/`.

The package itself is a session artifact and is not committed by this checkpoint. This branch records the promotion boundary and continuation state so a later integration cannot silently treat local experimental code as already authoritative runtime code.

## Deployment boundary

The currently linked Vercel `maestro-workforce` production deployment of `main` still fails before runtime startup because the repository has no recognized Python entrypoint. Existing draft PR #16 contains a broader browser/runtime entrypoint solution and remains a separate promotion stream. This audio-analysis checkpoint does not merge, duplicate, or bypass PR #16.

Heavy MIR/GPU inference must run as durable/background jobs on dedicated model workers or services, not inline in the Vercel request path. OpenAI evidence reasoning remains server-side and credential-bound.

## Next execution order

1. Provision one isolated analyzer environment at a time, starting with Beat This.
2. Run Bitter Thank You as the calibration fixture and compare model inferences with existing project evidence.
3. Add SongFormer, ChordMini/AMT, and CLAP results with per-adapter provenance and confidence.
4. Collect operator labels before interpreting embedding thresholds.
5. Run evidence-grounded audio-language reasoning only after deterministic/MIR records exist.
6. Use the resulting instrumentation for controlled Suno v6 prompt/renderer experiments.
7. Promote only replicated renderer behavior through `OBSERVATION -> CANDIDATE_HEURISTIC -> VALIDATED_HEURISTIC -> RENDERER_POLICY`.

**No canon/runtime promotion is requested by this checkpoint.**
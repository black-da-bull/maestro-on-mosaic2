# Council & Role Topology — Current v0.2 (O-16 derivation)
**DISC-11 closed: 5-council default + 13-worker expansion lens = designed coexistence, not rivals.**

## Default: five functional councils
Writer · Producer · Mix-Master · Technical · Strategy (era source: 02 master spec).

## Expansion lens: the 13 audio workers (rebirth roster)
Mo (standards·excellence) · Canon (structure·approvals) · Metro (feel·cultural truth) ·
Megazord (workflow·assembly) · Sibling (ethics·lore) · Sage (lyric motion) ·
Vanessa (believability) · Alan (arrangement·space) · Dave (pocket·groove) ·
Eldrik (tech feasibility) · Anva (replay·identity) · Melody Scout (motif·hook DNA) ·
Analog Confessor (aesthetic world — grounds the gospel/G-Funk/Southern-rap soul).
...
|---|---|---|
| STAFF.ROLE.THY_SME | THY | set_requiredness · approve_subkey/key/axis · score_criteria |
...
| STAFF.ROLE.PERFORMER | (none) | comment · vote · propose_change · demo_variants |
| STAFF.ROLE.PRODUCER | (none) | propose_change · approve_iteration_plan · schedule_meetings |

Approval states: TBD → SME_APPROVED | NEEDS_REVISION (categorical, stored). Meeting template
STAFF.MEETING.TEMPLATE.V1 "Axis Concurrent Review": REQ delta log review → resolve
requiredness=TBD → approve/return subkeys → update SEM run + scoreboard → baseline delta notes;
outputs: REQ.K3.Delta_Log append · status maps · SEM.K3 run append · milestone ⇒ SEG computed +
G-Card emitted (the interwoven model of 05, executable form). Tie-break/escalation: creative-axis
call = owning SME; policy = GOVERNANCE (block_pass/approve_exceptions); iteration order = PRODUCER.

## NAMED SEAT MAP — IMPORTED: PIN.V5B.COUNCIL.MATRIX.V1 (O16D fold)
...
cannot be overridden by taste. Gate authority: **V&V Marshal (Orchestrator-controlled)** runs SEG
feasibility, G-Card scoring, SE20 checklist, lock compliance, contradiction scan, cap checks —
cannot be overridden; any lyrics-lock risk triggers stop-the-line, not tie-break (COUNCIL_PROTOCOL
sha256 `d1c889f7d4734eeedd1409ec826b82b460d7fe9fa9bc48ef2502f9cc138ecc9d`). Stratification notes
(registered, not harmonized): the V&V Marshal (v5b era) and STAFF.ROLE.GOVERNANCE (personas draft)
are overlapping gate-authority constructs from different strata — correspondence adjudication =
operator (O-18 family). "Melony (Melody)" is the v5b-era seat name; the rebirth roster's
"Melody Scout" is a different stratum's name for the melody domain — both preserved.

## Overlay chronology (DEC-23: each version is an overlay on the system at its time)
...
The v4.5.5 production roster (RapCouncil, LyricForge, AI_Arranger.vx4, MixMaster_Ghost, …) is
PROVEN-era canon — preserved in stratigraphy, not merged with the rebirth roster.
Historical roster evidence: Yesterday-1 family (primary wrapper: eldrik.txt.txt, DEC-16).

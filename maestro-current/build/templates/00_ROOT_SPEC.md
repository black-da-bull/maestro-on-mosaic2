...
## Boundary (P-1)
Maestro is the independently shippable runtime. MOSAIC is a separate substrate (the empirical,
domain-neutral model of the target machines) connected through the explicit interface/portability
contract (08_PORTABILITY_ATP.md). Dev environments are ground; forensics is recovery tooling.
The "four planes" name Maestro's internal v.0 architecture only — never the top-level ontology.

## Runtime (the loop)
Pre-load nulls → blow-up intake (lyric lock engages) → sequential staffed fill, zero skips →
congruency round-robin (congruency, not turns) → executive red-pen → loop-until-pass → definitive
LOCK → FOIL reverse-pass (factoring rule = routing rule) into the derived surfaces → Suno.
Definitive lock precedes any reverse compilation. Surfaces freeze before packaging.
No distribution without linkage (the v5 loss is the canonical counterexample).
...
## Ship form (P-4)
This workspace-native product (files 00–10 + MANIFEST) ships first and serves as the accepted
specification and test oracle for the later executable application.

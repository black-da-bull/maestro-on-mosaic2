...
impact, lyric locks, and reviewer rules stay in runtime/maestro-workforce. Renderer
adherence observations are exposed as empirical adapter evidence and cannot mutate
Technical UST or promote themselves into canon.
"""
...
APP_ROOT = ROOT / "apps" / "maestro-browser-mvp"
RUNTIME_ROOT = ROOT / "runtime" / "maestro-workforce"
sys.path.insert(0, str(RUNTIME_ROOT))

import technical_ust_runtime as technical_ust  # noqa: E402
import workforce_runtime as workforce  # noqa: E402
    ...
    "controls": [
        {
            "id": "low_end_presence",
            "type": "presence",
            "label": "Low-end intent",
            "status": "supported_cross_corpus",
            "validation": "detect/presence plus listening confirmation",
        },
        {
            "id": "tempo_relation",
            "type": "relational",
            "label": "Relative / half-double tempo behavior",
            "status": "supported_more_reliably_than_exact_integer_lock",
            "validation": "relational timing comparison",
        },
        {
            "id": "ensemble_response",
            "type": "interaction",
            "label": "Ensemble / vocal-space response",
            "status": "supported_at_signal_proxy_level",
            "validation": "relationship and listening evidence",
        },
        {
            "id": "style_sensitivity",
            "type": "comparative",
            "label": "External Style sensitivity",
            "status": "supported_material_participation",
            "validation": "controlled comparative render",
        },
        {
            "id": "exact_tempo",
            "type": "exact_numeric",
            "label": "Exact integer BPM obedience",
            "status": "unresolved_not_reliable_enough_for_law",
            "validation": "measured BPM error/tolerance",
        },
        {
            "id": "semantic_timbre",
            "type": "semantic_timbre",
            "label": "Exact semantic timbre identity (for example B3/Leslie)",
            "status": "unresolved_narrow_fidelity_probe",
            "validation": "listening plus semantic/source evidence",
        },
        {
            "id": "deterministic_repeatability",
            "type": "repeatability",
            "label": "Deterministic exact repeatability",
            "status": "not_established",
            "validation": "controlled repeated renders",
        },
    ],
}
...
def app(environ, start_response):
    ...
    try:
        ...
        if method == "POST" and path == "/api/change-preview":
            ...
            runtime = technical_ust.TechnicalUSTRuntime(
                values=data.get("values") or {},
                lyrics_locked=bool(data.get("lyrics_locked")),
            )
            event = runtime.apply_change(
                worker_id,
                address,
                data.get("value"),
                operator_authorized_lyric_change=bool(data.get("operator_authorized_lyric_change")),
            )
            return _json(start_response, "200 OK", {
                "event": event,
    ...
    except Exception as exc:  # Fail closed; expose only class + message, not a traceback.
